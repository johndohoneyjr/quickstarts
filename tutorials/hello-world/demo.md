...
az group create \
  --name $RESOURCE_GROUP \
  --location $LOCATION
# Individual container apps are deployed to an Azure Container Apps environment. Create the environment
...
# the service (Node) runs as an app server on --target-port 3000 (the app port)
